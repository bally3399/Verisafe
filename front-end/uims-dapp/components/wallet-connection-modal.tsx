...
export default function WalletConnectionModal({ isOpen, onClose, onSuccess }: WalletConnectionModalProps) {
  ...
  const { connectWallet, isConnected, walletName, address } = useWallet()
  ...
  useEffect(() => {
    if (isOpen && isConnected && walletName === selectedWallet?.cardanoKey && address) {
      setTimeout(() => {
        onClose()
        onSuccess()
      }, 800)
    }
  }, [isConnected, selectedWallet, isOpen, onClose, onSuccess, walletName, address])
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {walletOptions.map((wallet) => (
                  <Card
                      key={wallet.id}
                      className={`cursor-pointer bg-slate-800/50 border ${
                          selectedWallet?.id === wallet.id ? "border-cyan-500 ring-2 ring-cyan-500" : "border-slate-700"
