...
interface WalletConnectionModalProps {
  ...
  onClose: () => void;
  onSuccess?: () => void;
  intendedScreen?: string; // Pass the intended screen from the parent
}

export default function WalletConnectionModal({ isOpen, onClose, onSuccess, intendedScreen }: WalletConnectionModalProps) {
  const { isConnected, connectWallet, error: walletError, setError } = useWallet();
  const [selectedWallet, setSelectedWallet] = useState<string | null>(null);
  const [isConnecting, setIsConnecting] = useState(false);
  ...
  const handleConnect = async () => {
    ...
    try {
      await connectWallet(selectedWallet.toLowerCase(), selectedWallet); // Use the hook's connectWallet
      if (isConnected) {
        if (onSuccess) {
          onSuccess(); // Trigger the success callback to update the parent
        }
        setTimeout(() => {
          onClose(); // Close the modal after a delay
        }, 1500);
      } else {
        throw new Error("Wallet connection failed or not detected.");
      }
    } catch (err) {
      console.error("Wallet connection error:", err);
      const wallet = walletOptions.find((w) => w.name === selectedWallet);
      if (wallet) {
        window.open(wallet.url, "_blank"); // Redirect to install page if wallet is not found or connection fails
      }
    } finally {
      setIsConnecting(false);
    }
  ...
  const walletOptions = [
    { name: "Nami", icon: "🦎", description: "Light wallet for Cardano", url: "https://namiwallet.io/" },
    { name: "Eternl", icon: "♾️", description: "Feature-rich Cardano wallet", url: "https://eternl.io/" },
    { name: "Flint", icon: "🔥", description: "Simple and secure wallet", url: "https://flint-wallet.com/" },
    { name: "Typhon", icon: "🌊", description: "Advanced Cardano wallet", url: "https://typhonwallet.io/" },
  ];
